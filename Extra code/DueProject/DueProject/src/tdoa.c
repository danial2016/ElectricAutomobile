...
void calculate_position(double *x, double *y, double *z)
{
	...
	double n2m = N/(2*M);
	double om = O/M; //nan arises if M = 0
	double sq = pow2(n2m) - om;
	if (sq < 0)
	{
		sq = (sq)*(-1);
	}
	double temp_z = n2m - sqrt(sq);
	*z = temp_z;
	*x = G * temp_z + H;
	*y = W * temp_z + J;
	...
	if (isnan(temp_z))
	{
		...
		//double tempImz = sqrt(pow2(cimagf(Imz)) + pow2(crealf(Imz)));	
		...
		*x = crealf(Imx);
		*y = crealf(Imy);
		*z = crealf(Imz); //cimagf(Imz)*-1;
		
		if (crealf(Imz) < 0)
		{
			*z = -crealf(Imz);
		}
		else if (crealf(Imz) > 2)
		{
			*z = ((uint32_t) crealf(Imz))%2;
		}
		else 
		{
			*z = crealf(Imz);
